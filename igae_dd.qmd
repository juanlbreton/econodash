...
#| include: false
# importa datos
suppressMessages(source("source/data_igae_b2018.R", echo = FALSE))
```
...
df02 <-
  df01 |>
  mutate(
    id_mes = str_c(mes, str_extract(year(fecha_final), "\\d{2}$"), sep = "_")
  ) |>
  nest(.by = concepto) |>
...
ulti_fecha <-
  df02 |>
  filter(
    concepto == "Indicador Global de la Actividad Económica" &
...
# value box
bslib::value_box(
  paste("Variación respecto a", format(ulti_fecha - years(1), "%B %Y")),
  theme = "primary",
...
bslib::value_box(
  ...
  theme = "primary",
  value = scales::percent(
    (mes_actual - mes_anterior) / mes_anterior,
    accuracy = 0.01
  )
)
```
...
tbl_varia <-
  tibble(
    ...
    año_ant = c(
      (prim_año_act - prim_año_ant) / prim_año_ant,
      (secu_año_act - secu_año_ant) / secu_año_ant,
      (terc_año_act - terc_año_ant) / terc_año_ant
    ),
    mes_ant = c(
      (prim_mes_act - prim_mes_ant) / prim_mes_ant,
...
# texto para caption
text_cap <-
  paste0("hasta ", format(ulti_fecha, "%b %Y"), ".")
...
df02 |>
  ...
  geom_line(
    data = df02 |>
      filter(concepto == "Actividades primarias"),
    aes(x = fecha_final, y = valor, group = concepto, color = "Act. Primarias"),
    alpha = 0.35
  ) +
  geom_line(
    data = df02 |>
  ...
  geom_line(
    data = df02 |>
      filter(concepto == "Indicador Global de la Actividad Económica"),
    aes(x = fecha_final, y = valor, group = concepto, color = "Global"),
    alpha = 0.85,
    linewidth = 0.85
  ) +
  scale_color_manual(
    ...
    values = c("#B22203", "#06B304", "#00238D", "#9003C2")
  ) +
  labs(
    title = "Desemepeño Histórico de las Series del IGAE",
    subtitle = paste(
      "Indicadores de actividad económica Global, Primaria, Secundaria y Terciaria",
      text_cap
    ),
  ...
  scale_x_date(
    breaks = seq(min(df02$fecha_final), max(df02$fecha_final), length.out = 10),
    date_labels = "%Y_%m"
  )
```
:::
...
text_fecha <-
  paste0("hasta ", format(ultim_fecha, "%b %Y"), ".")
...
df_global_02 <- df_global_01 %>%
  mutate(
    mes_sexenio = map(data, pon_num),
    escalado = map(data, calc_escala)
  ) %>%
  unnest(everything()) |>
  mutate(mes_sexenio = mes_sexenio - 1)
```
...
bslib::value_box(
  paste(
    "Crecimiento Acumulado del Sexenio Actual hasta",
    format(ultim_fecha, "%b %Y")
  ),
  theme = "primary",
  value = scales::percent(
    (ultimo_actual_sexe - inicio_actual_sexe) / inicio_actual_sexe,
    accuracy = 0.01
  )
)
```
...
df_global_02 %>%
  ...
  mutate(
    last_val = ifelse(
      mes_sexenio == max(mes_sexenio),
      as.character(presidente),
      NA
    )
  ) %>%
  ggplot(aes(x = mes_sexenio, y = escalado, color = presidente)) +
  geom_line(alpha = 0.65) +
  geom_hline(yintercept = 0, alpha = 0.46, color = "grey80") +
  scale_color_manual(
    name = "Presidente",
    values = c("#34495E", "#F39C12", "#16A085", "#8E44AD", "#9F0D08", "#002BD8")
  ) +
  # theme_breton() +
  theme(
    legend.position = "top",
  ...
  labs(
    title = "Indicador Anticipado del Desempeño de la Economía en cada Periodo Presidencial",
